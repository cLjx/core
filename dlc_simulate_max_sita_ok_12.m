...
simulate_OK_f_2=[];
for ininindex=1:2
    clearvars -EXCEPT ininindex simulate_OK_f_1 simulate_OK_f_2
    if ininindex==1
II=imread('dlc1.jpg');
    elseif ininindex==2
...
clearvars -EXCEPT Ou Ou1 ininindex simulate_OK_f_1 simulate_OK_f_2
% plot(Ou1)
% max(Ou1) max(Ou)
Ou=Ou-124;
Ou1=Ou1-124;
...
clearvars -EXCEPT Out1 Out ininindex simulate_OK_f_1 simulate_OK_f_2
% figure
% hold on
% plot(Out(:,1),Out(:,2));
% plot(Out1(:,1),Out1(:,2));
...
%simulate_OK_f
for simulate_f=3:30
    clearvars -EXCEPT Out1 Out simulate_f simulate_OK_f ...
        ininindex simulate_OK_f_1 simulate_OK_f_2
% simulate_OK(x,2);
for simulate_i=1:89
    clearvars -EXCEPT Out1 Out simulate_i simulate_OK simulate_f simulate_OK_f ...
        ininindex simulate_OK_f_1 simulate_OK_f_2
    sita_simulate=simulate_i/180*pi;
    f_simulate=simulate_f;
...
%add middle data end -----------------------------
sizeO1=size(O,1);
sita=sita_simulate;f=f_simulate*10^6;
x=0;y=170/250*100;
O_(1,:)=[x y];
back=0;
%test_sita
...
simulate_OK_f(f_simulate)=sum(simulate_OK(:,1));
f_simulate
end
% figure
if ininindex==1
simulate_OK_f_1=simulate_OK_f(3:30);
elseif ininindex==2
    simulate_OK_f_2=simulate_OK_f(3:30);
end
% plot((3:30),simulate_OK_f(3:30));
% grid on
% xlim([0 31])
% ylim([0 90])
% xlabel('f /MHz')
...
% for i=3:30
%     p(i-2,:)=[simulate_OK_f(i) i];
% end
% figure
% plot(p(:,1),p(:,2));grid on
% ylim([0 31])
% xlim([0 90])
% ylabel('最大可用频率 f_M_U_F/MHz')
% xlabel('仰角 \theta/°')
end
hold on
plot((3:30),simulate_OK_f_1);
plot((3:30),simulate_OK_f_2);
grid on
xlim([0 30])
ylim([0 90])
xlabel('f /MHz')
